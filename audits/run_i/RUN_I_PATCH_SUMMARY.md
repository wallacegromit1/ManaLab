...
| H-02 | `phase3_profiles.aggregate_profile_events`, `compare_profile_trials`, `component_population_id`; `statistics.stratified_paired_difference` | Six raw event-based ordered profile vectors; explicit per-turn and play/draw population weights; hand-calculated stratified paired fixtures | Not called by actual candidate-selection/validation stages; full sensitivity/profile overlap and absent population audits remain |
...
| H-04 | `statistics.paired_difference`, `holm_family`, `adaptive_paired_difference`; `metrics.uncertainty_aware_*`; `phase3_screening_ledger.ReversibleScreeningLedger` | Finite CI validation, keyed paired intervals, Holm, sequential Bonferroni, oriented frontier/regret; append-only family-registered evidence, protected identities, reversible adaptive-look disposition; C0-neutral pipeline probe | Complete production candidate/profile family registration, real stage-04–09 callers and raw-evidence handoff remain missing |
